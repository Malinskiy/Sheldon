...
public class Setter implements Generatable {
    ...
    private final PREF_TYPE PREFType;
    ...
    @Override public void generateCode(TypeSpec.Builder builder) throws ProcessingException {
        ...
        switch (PREFType) {
            case BOOLEAN:
                setterBuilder.addStatement(primitiveStatement("Boolean", prefName));
                break;
            case FLOAT:
                setterBuilder.addStatement(primitiveStatement("Float", prefName));
                break;
            case INT:
                setterBuilder.addStatement(primitiveStatement("Integer", prefName));
                break;
            case LONG:
                setterBuilder.addStatement(primitiveStatement("Long", prefName));
                break;
            case STRING:
                setterBuilder.addStatement(primitiveStatement("String", prefName));
                break;
            case OBJECT:
    ...
    private String primitiveStatement(String type, String prefName) {
